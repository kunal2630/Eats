...
const RestaurantItem = () => {
	...
	const menu =
		RestaurantItemList?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards?.filter(
			function (it) {
				return (
					it.card?.card?.["@type"] ===
	...
	const {
		...
		locality,
		cloudinaryImageId,
	} = RestaurantItemList?.cards[2]?.card?.card?.info || {};

	const delTime = Math.floor(
		RestaurantItemList?.cards[2]?.card?.card?.info?.sla?.deliveryTime / 2
	);
		...
		<>
			<div className="mx-auto w-[95%] md:w-[60%] p-4 relative">
				<div className="max-[768px]:flex-col flex justify-between">
					<div>
						<div className="">
							<p className="text-3xl text-[#282c3f] font-bold ">
								{name}
							</p>
						</div>
						<div className="mt-3 ">
				...
				<div className="relative">
					...
					{storeitems?.length > 0 ? (
						<Link to="/cart">
							<div
								className=" fixed  left-1/2 transform -translate-x-1/2 bg-orange-400 text-black hover:bg-[#F28C28] rounded hover:text-white p-2 text-xs md:text-sm hover:shadow-md transition-all cursor-pointer z-50   "
								style={{ top: "calc(100vh - 6%)" }}
							>
								<button className=" rounded-md text-white">
									{storeitems.length} items | View Cart
								</button>
							</div>
						</Link>
					) : (
